...
COMBINED_OUTPUT_DIR = f'./loso_results_combined_IMU_EMG_FIXED_ordered_GRU_random_state_{RANDOM_STATE}' 
...
STUDENT_OUTPUT_DIR = f'./loso_results_student_IMU_distilled_FIXED_GRU_random_state_{RANDOM_STATE}'
WANDB_PROJECT_NAME_STUDENT = f"IMU_Gesture_LOSO_Student_Distilled_FIXED_GRU_random_state_{RANDOM_STATE}"
KD_ALPHA = 0.3  
KD_TEMPERATURE = 2.0 
...
class CombinedGestureModel(nn.Module):
    def __init__(self, num_imu_sensors, imu_sequence_length, num_gestures,
        ...
        imu_gru_output_dim = 128 * 2  

        # Fusion and Classifier
        self.fc = nn.Sequential(
            nn.Linear(imu_gru_output_dim + self.emg_embedding_dim, 256), 
...
class StudentIMUModel(nn.Module):
    def __init__(self, num_imu_sensors, imu_sequence_length, num_gestures):
        ...
        # Classifier
        self.fc = nn.Sequential(
            nn.Linear(imu_GRU_output_dim, 256), # Only IMU features
            nn.ReLU(), nn.Dropout(0.5),
            nn.Linear(256, num_gestures)
        )
    ...
    def forward(self, imu_pixel_values=None, labels=None, **kwargs):
        ...
        hidden_imu = hidden_imu.view(self.imu_GRU.num_layers, 2, x_imu.size(0), self.imu_GRU.hidden_size)
        last_forward_imu = hidden_imu[-1, 0, :, :]
        last_backward_imu = hidden_imu[-1, 1, :, :]
        imu_representation = torch.cat([last_forward_imu, last_backward_imu], dim=1)
