...
def train_svm_for_loso(X_train, y_train, kernel_type='rbf'): 
    """Trains an SVM classifier for a LOSO fold."""
    
    model = SVC(C=1.0, kernel=kernel_type, gamma=GAMMA_TO_USE, 
                class_weight='balanced', random_state=RANDOM_STATE, probability=False)
    model.fit(X_train, y_train)
    return model

def evaluate_loso_results(all_y_true, all_y_pred, label_encoder, feature_names, all_feature_importances, output_dir, model_type="SVM"):
    ...
    if all_feature_importances:
        if len(set(len(fi) for fi in all_feature_importances)) > 1:
            ...
            plt.title(f"Top {top_n} Averaged Feature Importances/Coefficients (LOSO - Aggregated - {model_type})")
            y_label = "Mean Absolute Coefficient (Linear SVM)" if model_type == "SVM" else "Mean Gini Importance"
            sns.barplot(x=[feature_names[i] for i in indices[:top_n]], y=avg_importances[indices][:top_n], palette="viridis")
            plt.xticks(rotation=90)
            plt.ylabel(y_label)
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, f"loso_avg_feature_importances_AGGREGATED_{model_type}.png"))
            plt.close()
            print(f"Averaged LOSO feature importances/coefficients plot (Aggregated - {model_type}) saved.")

            importance_df = pd.DataFrame({'feature': feature_names, 'importance_or_coeff': avg_importances})
            importance_df = importance_df.sort_values(by='importance_or_coeff', ascending=False).reset_index(drop=True)
            importance_df.to_csv(os.path.join(output_dir, f"loso_avg_feature_importances_AGGREGATED_{model_type}.csv"), index=False)
            print(f"Averaged LOSO feature importances/coefficients (Aggregated - {model_type}) saved to CSV.")
    else:
        print(f"No feature importances/coefficients collected to average for aggregated report (e.g., SVM with non-linear kernel used, or no models trained).")


def save_per_participant_metrics(per_participant_metrics_list, gesture_class_names, output_dir, model_type="SVM"):
...
def main_gesture_recognition_loso():
    print("--- Starting LOSO Gesture Recognition Pipeline (SVM) ---") 
    
    svm_kernel_to_use = SVC_KERNEL_TO_USE

    print(f"Using SVM with kernel: {svm_kernel_to_use}")
    ...
    for train_idx, test_idx in logo.split(full_df, groups=groups_for_loso):
        ...
        print(f"\n--- Fold {fold_num}/{len(participants)}: Testing on Participant {current_test_participant} (SVM) ---")
        ...
        model = train_svm_for_loso(X_train_scaled, y_train_encoded, kernel_type=svm_kernel_to_use)
        ...
        if svm_kernel_to_use == 'linear' and hasattr(model, 'coef_'):
            ...
            if len(importances) == len(feature_names):
                all_fold_feature_importances.append(importances)
            else:
                print(f"Warning: Mismatch SVM .coef_ length ({len(importances)}) vs features ({len(feature_names)}) for fold {fold_num}.")
        elif svm_kernel_to_use != 'linear':
            pass 
        ...
        print(f"Fold {fold_num} (Participant {current_test_participant}, SVM) Accuracy: {acc_fold:.4f}. Full report metrics stored.")
    ...
    if not all_y_true_encoded_aggregated:
        ...
        evaluate_loso_results(all_y_true_encoded_aggregated, all_y_pred_fold_aggregated, le, 
                              feature_names, all_fold_feature_importances, MODEL_OUTPUT_DIR, model_type="SVM")

    save_per_participant_metrics(per_participant_metrics_list, le.classes_.astype(str), MODEL_OUTPUT_DIR, model_type="SVM")

    print("\n--- LOSO Gesture Recognition Pipeline (SVM) Finished ---") 
