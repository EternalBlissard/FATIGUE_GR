...
# TODO: PATHS
FEATURES_BASE_DIR = '/raid/home/kirtil/Fatigue_Study/Delsys Processed Data/features_IMU_ordered1'
MODEL_OUTPUT_DIR = f'/raid/home/kirtil/Fatigue_Study/Delsys Processed Data/neurips/KNN_outputs_IMU_ordered_LOSO_detailed_random{RANDOM_STATE}_check'
# FEATURES_BASE_DIR = 'path/to/IMU/features'
# MODEL_OUTPUT_DIR = f'path/to/output/directory_random{random_state}'
os.makedirs(MODEL_OUTPUT_DIR, exist_ok=True)


# TODO: KNN parameters
KNN_N_NEIGHBORS_TO_USE = 5
WEIGHTS_TO_USE = 'distance'
METRIC_TO_USE = 'minkowski'
P_TO_USE = 2
N_JOBS_TO_USE = -1
...
def train_knn_for_loso(X_train, y_train, n_neighbors=5):
    """Trains a KNN classifier for a LOSO fold."""
    model = KNeighborsClassifier(n_neighbors=KNN_N_NEIGHBORS_TO_USE, weights=WEIGHTS_TO_USE, 
                                 metric=METRIC_TO_USE, p=P_TO_USE, n_jobs=N_JOBS_TO_USE) 
    model.fit(X_train, y_train)
    return model

def evaluate_loso_results(all_y_true, all_y_pred, label_encoder, feature_names, all_feature_importances, output_dir, model_type="KNN"):
    """Evaluates aggregated LOSO results and saves them, including gesture-wise accuracy."""
    print(f"\n--- Aggregated LOSO Evaluation ({model_type}) ---")
    ...
    with open(os.path.join(output_dir, f"loso_overall_accuracy_AGGREGATED_{model_type}.txt"), "w") as f:
    ...
    path_gesture_wise_acc = os.path.join(output_dir, f"loso_gesture_wise_accuracy_AGGREGATED_{model_type}.txt")
    ...
    with open(os.path.join(output_dir, f"loso_classification_report_AGGREGATED_{model_type}.txt"), "w") as f:
        f.write(f"Overall LOSO Accuracy (Aggregated): {accuracy:.4f}\n\n")
        f.write(full_report_str)
    print(f"Full classification report saved to: {os.path.join(output_dir, f'loso_classification_report_AGGREGATED_{model_type}.txt')}")
    ...
    plt.title(f'Overall LOSO Confusion Matrix (Aggregated - {model_type})')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"loso_confusion_matrix_AGGREGATED_{model_type}.png"))
    plt.close()
    print(f"Overall LOSO Confusion matrix (Aggregated - {model_type}) saved.")
    ...
    if all_feature_importances: 
        ...
        if len(set(len(fi) for fi in all_feature_importances)) > 1:
            ...
            plt.title(f"Top {top_n} Averaged Feature Metrics (LOSO - Aggregated - {model_type})")
            y_label = "Mean Feature Metric Value" 
            if model_type == "RF": y_label = "Mean Gini Importance"
            elif model_type == "SVM": y_label = "Mean Absolute Coefficient"

            sns.barplot(x=[feature_names[i] for i in indices[:top_n]], y=avg_importances[indices][:top_n], palette="viridis")
            plt.xticks(rotation=90)
            plt.ylabel(y_label)
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, f"loso_avg_feature_metrics_AGGREGATED_{model_type}.png")) 
            plt.close()
            print(f"Averaged LOSO feature metrics plot (Aggregated - {model_type}) saved.")

            importance_df = pd.DataFrame({'feature': feature_names, 'metric_value': avg_importances}) 
            importance_df = importance_df.sort_values(by='metric_value', ascending=False).reset_index(drop=True)
            importance_df.to_csv(os.path.join(output_dir, f"loso_avg_feature_metrics_AGGREGATED_{model_type}.csv"), index=False) 
            print(f"Averaged LOSO feature metrics (Aggregated - {model_type}) saved to CSV.")
    else:
        print(f"No direct feature importances collected to average for aggregated report (as expected for {model_type}, or no models trained).")


def save_per_participant_metrics(per_participant_metrics_list, gesture_class_names, output_dir, model_type="KNN"):
    ...
    print(f"\n--- Saving Per-Participant Metrics ({model_type}) ---")
    
    path_txt = os.path.join(output_dir, f"loso_per_participant_classification_reports_{model_type}.txt")
    ...
    if summary_data:
        ...
        path_csv = os.path.join(output_dir, f"loso_per_participant_metrics_summary_{model_type}.csv")
...
def main_gesture_recognition_loso():
    print("--- Starting LOSO Gesture Recognition Pipeline (KNN) ---")
    
    
    print(f"Using KNN with n_neighbors: {KNN_N_NEIGHBORS_TO_USE}")
    ...
    all_fold_feature_importances = [] # KNN doesn't have direct feature importances, so this will remain empty
    ...
    for train_idx, test_idx in logo.split(full_df, groups=groups_for_loso):
        ...
        print(f"\n--- Fold {fold_num}/{len(participants)}: Testing on Participant {current_test_participant} (KNN) ---")
        ...
        model = train_knn_for_loso(X_train_scaled, y_train_encoded, n_neighbors=KNN_N_NEIGHBORS_TO_USE)
        ...
        print(f"Fold {fold_num} (Participant {current_test_participant}, KNN) Accuracy: {acc_fold:.4f}. Full report metrics stored.")
    ...
    if not all_y_true_encoded_aggregated:
        ...
        evaluate_loso_results(all_y_true_encoded_aggregated, all_y_pred_fold_aggregated, le, 
                              feature_names, all_fold_feature_importances, MODEL_OUTPUT_DIR, model_type="KNN")

    save_per_participant_metrics(per_participant_metrics_list, le.classes_.astype(str), MODEL_OUTPUT_DIR, model_type="KNN")

    print("\n--- LOSO Gesture Recognition Pipeline (KNN) Finished ---")
